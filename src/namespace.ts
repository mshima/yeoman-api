...
export function parse(complete: string): YeomanNamespace | undefined {
 ...
 * Parse a namespace
 */
function parseNamespace(complete): YeomanNamespace | undefined {
  if (typeof complete !== 'string') {
    return undefined;
  }

  const parsed = parse(complete);
  return parsed ? new YeomanNamespace(parsed) : undefined;
}
...
export function toNamespace(namespace: string | YeomanNamespace): YeomanNamespace | undefined {
  return isNamespaceObject(namespace) ? (namespace as YeomanNamespace) : parseNamespace(namespace);
}
 ...
 * @private
 * @param  namespace
 * @return True if namespace is a YeomanNamespace
 */
